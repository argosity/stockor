...
module Skr::Workspace::EJS
  ...
  class << self
    ...
    protected
      ...
      def generate_try_catches(source)

        # Find all variables the template is trying to check in if conditions
        checked_vars = source.scan(/if\s*\(\s*!?([\w\-_]*)\s*\)/).flatten.uniq

        # Generate the try-catches
        try_catches = ''

        if checked_vars
            checked_vars.each do |checked_var|
                try_catches << "try {#{checked_var};} catch(e) { if (e instanceof ReferenceError) var #{checked_var};}\n"
            end
        end
